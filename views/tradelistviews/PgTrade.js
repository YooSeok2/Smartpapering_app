...
export default function PgTrade({statusArray, pgOrders, refreshing, onRefresh}){
        ...
        if(sameStatus.length > 0){
            switch(sameStatus[0].status){
                case '시공접수' :
                    return (
                        <View style={styles.statusBox}>
                            <Image source = {require(`../../assets/completed-task.png`)} style={styles.logoImg} />
                ...
                case '시공진행' :
                    return (
                        <View style={styles.statusBox}>
                            <Image source = {require(`../../assets/wrench.png`)} style={styles.logoImg} />
                            <Text style={[styles.status, {color : sameStatus[0].color}]}>{sameStatus[0].status}</Text>
                        </View>
                    );
                default : break;
            ...
            ListEmptyComponent = {()=>{
                return(
                    <View style={styles.noTradeBox}>
                        <Text style={styles.noTrade}>진행중인 시공이 없습니다</Text>
                    </View>
                )
            }}
            numColumns={2}
