...
export default function PgWithdraw({statusArray, pgWithdraws, refreshing, onRefresh}){
        ...
        if(sameStatus.length > 0){
            return (
                <View style={styles.statusBox}>
                    <Image source = {require(`../../assets/dollar.png`)} style={styles.logoImg} />
    ...
    const renderItem=({item})=>{ 
        return(
            <View style={styles.withdrawBox}>
                ...
                <View style={styles.statusDetail}>
                    <Text style={styles.statusTitle}>금액 : </Text>
                    <Text style={styles.price}>{numberWithCommas(item.withdraw_price)}원</Text>    
                </View>
                <View style={styles.statusDetail}>
                    <Text style={styles.statusTitle}>접수일 : </Text>
            ...
            ListEmptyComponent = {()=>{
                return(
                    <View style={styles.noWithdrawBox}>
                        <Text style={styles.noWithdraw}>접수된 출금이 없습니다</Text>
                    </View>
                )
            }}
            numColumns={2}
